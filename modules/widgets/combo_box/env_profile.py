...
from modules.widgets.combo_box.items_env_profile import combo_box_items_env_profiles
...
class WidgetEnvProfile(QComboBox):
    def __init__(self, parent=None):
        super().__init__(parent)
        
        # Set object name if needed
        self.setObjectName("template_env_profile")

        # Load items from the imported list
        self.addItems(combo_box_items_env_profiles)

        # Set default properties
        self.setCurrentText(self.get_default_env_profile())
        self.setEditable(False)

    def get_widget_parameters(self):
        widget_parameters = {
            "widget_name": "template_env_profile",
            "widget_type": "QComboBox",
            "widget_label_name": "Environment Profile: ",
            "widget_default_value": self.get_default_env_profile(),
            "widget_placeholder_value": "",
            "widget_item_values": "items_env_profile.py",
            "widget_read_only": False
        }
        return widget_parameters

    def get_default_env_profile(self):
        """
        ...
        try:
            with open(json_file_path, 'r') as file:
                data = json.load(file)
            env_profile = data.get("Environment Profile: ")
            return env_profile
        except:
            # Return a default value in case of an error
            return "logik_default"
